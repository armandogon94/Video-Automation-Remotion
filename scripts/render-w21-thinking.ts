#!/usr/bin/env node
/**
 * W21 AGENT THINKING renderer.
 *
 * Renders the 2026-05-18 "Gemini 3.2 Flash leak" voiceover onto the
 * AgentThinking9x16 composition (the breathing-orb thinking shot) in
 * BOTH `cream` and `dark` palettes. The orb visually represents "Google
 * processing/leaking the model" — a calm-before-the-news beat.
 *
 * Outputs:
 *   output/2026-05-18-gemini-3-2-flash-leak/thinking-cream/master.mp4
 *   output/2026-05-18-gemini-3-2-flash-leak/thinking-cream/preview-t5s.jpg
 *   output/2026-05-18-gemini-3-2-flash-leak/thinking-dark/master.mp4
 *   output/2026-05-18-gemini-3-2-flash-leak/thinking-dark/preview-t5s.jpg
 */
...
async function main() {
  ...
  for (const palette of palettes) {
    const variantDir = path.join(OUT_DIR, `thinking-${palette}`);
    fs.mkdirSync(variantDir, { recursive: true });

    const inputProps = {
      audioUrl: publicAudioName,
      wordTimings,
      caption: "Google está cocinando algo...",
    ...
    // Preview at t=5s — mid-breathing-cycle.
    const jpgFile = path.join(variantDir, "preview-t5s.jpg");
    await execa("ffmpeg", [
      "-y", "-ss", "5", "-i", masterFile,
      "-frames:v", "1", "-q:v", "2",
      jpgFile,
  ...
  console.log("\n✓ W21 AgentThinking render complete");
...
main().catch((err) => {
  console.error("\n✗ W21 AgentThinking render failed:", err);
  process.exit(1);
});
