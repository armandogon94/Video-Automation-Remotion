#!/usr/bin/env node
/**
 * W21 SPARKLINE renderer.
 *
 * Renders the 2026-05-18 "Gemini 3.2 Flash leak" voiceover onto the
 * Sparkline9x16 composition — a fabricated "Gemini cost-per-token trajectory"
 * series showing the dramatic price drop from $3.00 down to $0.25.
 *
 * Output: output/2026-05-18-gemini-3-2-flash-leak/sparkline/master.mp4 + preview JPG
 */
...
async function main() {
  ...
  const variantDir = path.join(OUT_DIR, "sparkline");
  fs.mkdirSync(variantDir, { recursive: true });

  // Fabricated Gemini cost-per-token trajectory ($/1M input tokens) — Q1 2024
  // through Q2 2026. Shows the dramatic ~12× price drop that culminates in the
  // $0.25 leak. Sparkline draws latest value at the right (the current $0.25).
  const inputProps = {
    audioUrl: publicAudioName,
    wordTimings,
    kicker: "GEMINI COST/M TOKENS",
    title: "Precio Gemini Flash · 8 trimestres",
    data: [3.00, 2.50, 1.80, 1.20, 0.80, 0.50, 0.30, 0.25],
    valueFormat: "${v}",
    decimals: 2,
    showAreaFill: true,
    showTrendBadge: true,
    drawDurationSeconds: 1.4,
  ...
  // t=2.5s — line has fully drawn (1.4s) and end-dot has pulsed + trend badge in.
  const jpgFile = path.join(variantDir, "preview-t2.5s.jpg");
  await execa("ffmpeg", [
    "-y", "-ss", "2.5", "-i", masterFile,
    "-frames:v", "1", "-q:v", "2",
    jpgFile,
  ], { stdio: "ignore" });
...
main().catch((err) => {
  console.error("\n✗ W21 Sparkline render failed:", err);
  process.exit(1);
});
