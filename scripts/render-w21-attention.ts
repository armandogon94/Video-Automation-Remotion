#!/usr/bin/env node
/**
 * W21 ATTENTION HEATMAP renderer.
 *
 * Renders the 2026-05-18 "Gemini 3.2 Flash leak" voiceover onto the
 * AttentionHeatmap9x16 composition (a strong-diagonal attention matrix
 * over the row/col tokens of the leak headline) in BOTH `cream` and
 * `dark` palettes. Pedagogical "this is what Gemini is doing under the
 * hood" visual.
 *
 * Outputs:
 *   output/2026-05-18-gemini-3-2-flash-leak/attention-cream/master.mp4
 *   output/2026-05-18-gemini-3-2-flash-leak/attention-cream/preview-t10s.jpg
 *   output/2026-05-18-gemini-3-2-flash-leak/attention-dark/master.mp4
 *   output/2026-05-18-gemini-3-2-flash-leak/attention-dark/preview-t10s.jpg
 */
...
async function main() {
  ...
  // Strong-diagonal attention pattern — the editorial default that reads
  // as "each token attends primarily to itself, with secondary cross-attention".
  const tokens = ["Gemini", "3.2", "Flash", "es", "muy", "rápido"];
  const weights = [
  ...
  for (const palette of palettes) {
    const variantDir = path.join(OUT_DIR, `attention-${palette}`);
    ...
    // Preview at t=10s — well after the 1.6s scan-fill animation has completed.
    const jpgFile = path.join(variantDir, "preview-t10s.jpg");
    await execa("ffmpeg", [
      "-y", "-ss", "10", "-i", masterFile,
      "-frames:v", "1", "-q:v", "2",
      jpgFile,
    ], { stdio: "ignore" });
  ...
  console.log("\n✓ W21 AttentionHeatmap render complete");
...
main().catch((err) => {
  console.error("\n✗ W21 AttentionHeatmap render failed:", err);
  process.exit(1);
});
